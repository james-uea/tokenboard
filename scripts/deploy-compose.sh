...
Environment overrides:
  COMPOSE_FILE, ENV_FILE, APP_SERVICE, READY_URL, WAIT_TIMEOUT, PULL_SERVICES
USAGE
}
...
PULL_SERVICES="${PULL_SERVICES:-postgres cloudflared}"
...
if [[ -z "$ENV_FILE" || ! -f "$ENV_FILE" ]]; then
  ...
  echo "Create it from .env.example and fill in deployment secrets." >&2
  exit 1
fi
...
LOCK_DIR="${TMPDIR:-/tmp}/tokenboard-deploy-compose.lock"
...
echo "Deploying Tokenboard from $REPO_ROOT"

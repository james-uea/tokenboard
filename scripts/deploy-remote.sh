...
usage() {
  cat >&2 <<'USAGE'
Usage:
  scripts/deploy-remote.sh <ssh-target> [remote-dir]

Examples:
  scripts/deploy-remote.sh deploy@example.com
  scripts/deploy-remote.sh deploy@192.168.1.50 /opt/tokenboard

Copies the current working tree to the target server using one shared SSH
connection. Existing env files on the target are preserved.
USAGE
}

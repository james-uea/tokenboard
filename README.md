...
[DEPLOYMENT.md](./DEPLOYMENT.md) for the full Docker Compose deployment, firewall,
...
| `BACKUP_PASSPHRASE` | unset | Optional passphrase for encrypted database backups |
| `BACKUP_RETENTION_DAYS` | `30` | Optional remote backup retention window |
| `S3_ENDPOINT_URL` | unset | Optional S3-compatible endpoint for backups |
| `S3_BUCKET` | unset | Optional S3-compatible backup bucket |
| `S3_PREFIX` | `postgres` | Optional backup object prefix |
| `S3_REGION` | `auto` | Optional S3 region |
| `S3_ACCESS_KEY_ID` | unset | Optional S3-compatible backup access key |
| `S3_SECRET_ACCESS_KEY` | unset | Optional S3-compatible backup secret |
